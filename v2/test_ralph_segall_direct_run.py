...
def main():
    ...
    with open("ralph_segall_report_new.json", "w") as f:
        f.write(json_dumps_with_alerts(result, indent=2))
    
    logger.info("Report saved to ralph_segall_report_new.json")
    ...
    if "name_evaluation" in result:
        print(f"\nName evaluation status: {result['name_evaluation'].get('compliance', 'Unknown')}")
    ...
    if "final_evaluation" in result:
        print(f"\nFinal evaluation compliance: {result['final_evaluation'].get('compliance', 'Unknown')}")
        print(f"Overall risk level: {result['final_evaluation'].get('overall_risk_level', 'Unknown')}")
        print(f"Recommendations: {result['final_evaluation'].get('recommendations', 'Not provided')}")
